...
| AC4 | `tests/Unit/Signing/ClientBoundsTest.php` :: "caps the service error text it copies into an exception", "still reports a short service error in full" | `src/Core/Support/ServiceError.php` `fromBody()` (moved out of `SigningServiceSigner::extractError()` by SPEC-031, which extended the cap to the reader) |
