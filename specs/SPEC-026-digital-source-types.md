...
| AC4 *(amended by SPEC-028 — the refusal is lifted; what survives is that an editing term never rides on `c2pa.created`)* | `tests/Unit/Manifest/DigitalSourceTypeTest.php` :: "classifies a source type that describes an operation on an existing asset", "never emits a created action for an editing term" | `src/Core/Manifest/DigitalSourceType.php` `requiresIngredient()`, `src/Core/Manifest/Manifest.php` `requiresParentAsset()` |
