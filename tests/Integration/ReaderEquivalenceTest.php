 ...
 * service carries **0.90.16** (verified 2026-08-21), so the same asset is read by
 ...
 *
 * AC2 compares two ENGINES — c2pa-rs 0.89.0 against 0.90.16. Comparing them
...
it('agrees with the signing-service reader on every public accessor', function () {
    ...
    // the extension against 0.90.16 in the service is the reason to expect one.
    foreach ($viaExtension as $accessor => $value) {
        expect($value)->toBe(
            $viaService[$accessor],
            sprintf(
                'readers disagree on %s(): extension (c2pa-rs 0.89) says %s, service (0.90.16) says %s',
                $accessor,
                json_encode($value),
...
it('agrees that a declared media type is advisory, not enforced', function () {
    ...
    // against the service's 0.90.16 — we learn it here rather than from a user
